...
"""Unified paged attention, flat grid form.

Grid `[num_splits, num_q_tiles, num_kv_heads]`. The query axis is flattened over
the sync-free host bound in `kernel_common.unified_factory`, with
`(seq_idx, q_offset)` recovered on device by binary search over
`query_start_loc`. That removes the 3-D form's empty tiles on decode-heavy
batches (87% empty on 255 decodes + 1 prefill).

Output-identical to `kernel_grid.py` by construction: everything from the
`q_start + q_offset < q_end` guard down is the same computation with `q_offset`
in place of `tile_q.begin`. Any correctness-bearing change must be made in both.
"""
...
def _kernel_unified_flat(
    ...
    # Registered minimum must stay 1 and must equal `kernel_common._Q_BLOCK_MIN`,
    # which the host grid bound divides by. The maxes are the constexpr budgets;
    # see `kernel_common` for why they cannot be inline literals.
    q_block = hl.register_block_size(1, m_budget)
    block_p = hl.register_block_size(2, p_budget)
    # Split-KV. Buys occupancy on decode, where the grid is only
    # `num_splits * num_seqs * num_kv_heads`: at nseq=1/nkv=8 that is 8 blocks on
    # 132 SMs at 1 split versus 256 at 32. Neutral on prefill, where the q-tile
    # axis already fills the machine, so the tree picks 1 for every leaf with
    # `q_block >= 32`. Phase 1 writes un-normalized partials; `_kernel_combine`
    # does the online-softmax merge.
    ...
    for tile_s, tile_t, tile_g in hl.tile(
        ...
        if q_start + q_offset < q_end:
            ...
            # Interior/diagonal boundary, rounded DOWN to a whole `block_p`
            # relative to `split_lo`: `hl.tile`'s tail tile is partial but the
            # gather reshape below uses the FULL block size, so an unrounded
            # boundary loads real KV past the causal range.
            n_page_split = split_lo + ((n_page_split - split_lo) // block_p) * block_p

            # Interior pages: entirely below the diagonal, so no mask at all --
            # no comparison tensor, no `where`, and `m_i` can never be -inf. The
            # diagonal tile count is near-constant (perimeter) while the interior
            # grows with context (area), so this branch takes 79-99% of pages.
            # K/V are gathered straight into the `hl.dot` operands; materializing
            # a dense padded tile instead costs 557 KB of smem at page 16 /
            # block_p 32, over the 232 KB H100 opt-in limit.
            for tile_p in hl.tile(split_lo, n_page_split, block_size=block_p):
                ...
                # Base-2 softmax: `exp2` is a single hardware instruction, so
                # `_LOG2_E` is folded into the scale once instead of paying a
                # multiply per element inside `exp`.
            ...
            # Diagonal pages: the only ones needing the causal mask, plus the
            # sequence-end and split-end bounds and the query-row guard.
