"""
Unified paged attention, 3-D grid form (plan stage 1).

Grid `[num_seqs, num_kv_heads, max_seqlen_q]` -- the shape `kernel_prefill.py:99-112`
uses today. Every row is over-tiled up to the batch maximum and the per-row guard
discards the surplus, so in R1 (255 decode rows + 1 prefill row) 87% of launched tiles
exit immediately. `kernel_flat.py` is the alternative that fixes that; this file is its
fallback and its correctness reference, and the two must stay output-identical (stage 2
is a pure performance change).

Its own AOT artifact is `_helion_aot_kernel_grid_cuda_sm90.py` -- see `kernel_common.py`
for why the file split is what keeps the two trees apart.
"""
...
# ---------------------------------------------------------------------------
# Phase 1 -- attention over each (row, kv_head, m_block) tile
# ---------------------------------------------------------------------------
def _kernel_unified(
    ...
    # Grid. `grid_extent` is `max_seqlen_q`; the q axis is tiled by `q_block`, so every
    # row is over-tiled up to the batch maximum and the guard below discards the rest.
    # The split axis goes FIRST, matching both predecessors -- `kernel_tune.py:120`
    # (`hl.grid([num_splits, num_seqs, num_kv_heads])`) and `kernel_flash.py:182-184`,
    # whose 4-D grid this is exactly. At `num_splits == 1` it degenerates to the stage-1
    # 3-D grid.
    for tile_s, s_idx, tile_g, tile_q in hl.tile(
        ...
        # FA3 bottom-right causal, no offset fixup (`hopper/block.h:23-33`). Floored at
        # 0 (INVARIANT 7): negative context is a silent NaN. No predecessor clamps this.
        context_len = torch.clamp(seq_len - (q_end - q_start), min=0)

        # Tail-padded rows have `q_end == q_start`; the 3-D grid also over-tiles every
        # row shorter than `grid_extent`, which is what makes a qlen=1 row legal here.
...
# DUPLICATED here and in `kernel_flat.py` rather than shared from one file. Helion names
...
# `_helion_aot_kernel_grid_*`. One shared file instead put both in
# `_helion_aot_kernel_combine_*`, which the second AOT run overwrote. The price is a
# second body that must be kept identical to `kernel_flat.py`'s -- the same rule the
